...
class Lepton(object):
  ...
  def __init__(self, spi_dev = "/dev/spidev3.0"):
    #self._spi_dev = spi_dev
    self._spi_dev = "/dev/spidev3.0"
  ...
  def capture_segment(handle, xs_buf, xs_size, capture_buf):
    ...
    #print "capture_segment" , handle
    while (capture_buf[0] & 0x000f) == 0x000f: # byteswapped 0x0f00
      ioctl(handle, iow, xs_buf, True)
      
      #rint (' '.join(hex(x) for x in capture_buf))
      #print "print xs_buf size, ",xs_size
      #for k in range(1,32):	
       # print (xs_buf[k], end=" ")
      #print (' '.join(str(x) for x in xs_buf))
