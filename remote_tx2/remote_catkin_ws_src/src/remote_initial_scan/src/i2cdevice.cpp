...
int I2CDevice::writeRegisterblock(unsigned int registerAddress, unsigned char *value, int buffersize){

   unsigned char buffer[buffersize+1];
   buffer[0] = registerAddress;
   memcpy( &(buffer[1]), value, buffersize);
   buffersize=buffersize+1;
   if(::write(this->file, buffer, buffersize)!=buffersize){
      perror("I2C: Failed write to the device\n");
      return 1;
   }
   return 0;
...
int I2CDevice::writeblock(unsigned char *value, int buffersize){

   if (::write(this->file, value, buffersize)!= buffersize){
      perror("I2C: Failed to write to the device\n");
      return 1;
   }
   return 0;
...
unsigned char I2CDevice::readbyte(void){
   //this->write(registerAddress);
   unsigned char buffer[1];
   if(::read(this->file, buffer, 1)!=1){
      perror("I2C: Failed to read in the value.\n");
...
unsigned char* I2CDevice::readLine(unsigned int number){
	//this->write(fromAddress);
	unsigned char* data ;
    if(::read(this->file, data, number)!=(int)number){
       perror("IC2: Failed to read in the full buffer.\n");
