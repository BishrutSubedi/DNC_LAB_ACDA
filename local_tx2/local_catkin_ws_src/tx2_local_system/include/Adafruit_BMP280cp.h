...
bool Adafruit_BMP280::begin() {
    ...
    BARO_device =  new I2CDevice (1,0x77);
...
{
    BARO_device->writeRegister(reg, value);
}
...
uint8_t Adafruit_BMP280::read8(byte reg)
{
    ...
    value= BARO_device->readRegister(reg);
    return value;
...
{
    uint16_t value;
    unsigned char * data=BARO_device->readRegisters( 2, reg);
    value=(*data)<<8 | *(data+1);
...
{
    uint32_t value;
    unsigned char * data=BARO_device->readRegisters(3,reg);
    value=*(data)<<8 |*(data+1);
